...
public class FileContent {
    ...
    private static List<String> filesWithCStyleComments = List.of("c","java","kt","cs","cpp","h","js","ts","swift","m","mm","r", "go","scala","sc","dart", "dm", "groovy", "gvy", "gy", "gsh");
    private static List<String> filesWithShellStyleComments = List.of("sh","bash","zsh");
    private static List<String> markupFiles = List.of("html","htm","xml");
    private static List<String> cssFiles = List.of("css","sass","scss","less");
    private static List<String> filesWithPythonStyleComments = List.of("py","ex","exs");
    ...
    public static boolean isBlockCommentFinished(String line, String fileExtension) {
        String regex = filesWithCStyleComments.contains(fileExtension)
                || List.of("sql","php").contains(fileExtension)
    ...
    public static boolean isInlineComment(String line, String fileExtension) {
        String regex = filesWithCStyleComments.contains(fileExtension)
                ...
                : filesWithShellStyleComments.contains(fileExtension)
                || filesWithPythonStyleComments.contains(fileExtension)
                || List.of("rb","pl","coffee").contains(fileExtension)
                ? "^\\s*#"
                : fileExtension.equals("php")
                ? "^\\s*(#|//)"
                : List.of("lua","sql").contains(fileExtension)
                ? "^\\s*--"
                : null;
        if (regex == null) return false;
        return matchString(line, regex);
